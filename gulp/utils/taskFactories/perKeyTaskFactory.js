...
module.exports = (namespace, task, {
  ...
  (config) => {
    ...
    const taskFactory = (tasksNamespace, taskAction, subtaskConfig) => {
      const tasks = Object.keys(subtaskConfig)
        .reduce(
          (accumulated, key) => {
            ...
            gutil.log(gutil.colors.red(`[${key}] invalid config, fix and retry.`));
            return { ...accumulated };
          },
          {}
        );
    ...
    switch (typeof task) {
      ...
      default:
        gutil.log(gutil.colors.red(`[${namespace}] provided task is not a function or an object, fix and retry.`));
        return {};
    }
  }
);
